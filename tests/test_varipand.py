...
def test_expanding_a_pattern_without_variations_returns_the_pattern():
    assert list(expand(["Hello World!"], DEFAULT_CONFIG)) == ["Hello World!"]


def test_every_input_pattern_is_expanded():
    assert list(expand(["Hello World!", ""], DEFAULT_CONFIG)) == [
        "Hello World!", ""]
...
def test_supports_alternative_start_and_end_delimiters():
    assert list(expand(["Hello [ World ]!", ], ALTERNATIVE_CONFIG_1)) == [
        "Hello World!"]
...
def test_returns_a_new_phrase_for_each_variant():
    assert list(expand(["Hello (World/You)(!/)", ], DEFAULT_CONFIG)) == [
        "Hello World!",
        "Hello You!",
        "Hello World",
        "Hello You",
...
def test_returns_a_new_phrase_for_each_variant_with_alternative_delimiter():
    assert list(expand(["Hello [ World, You ][ !,  ]", ], ALTERNATIVE_CONFIG_1)) == [
        "Hello World!",
        "Hello You!",
        "Hello World",
        "Hello You",
